...
/**
 * Flèche éphémère, jamais enregistrée : la liaison en cours (du bouton au pointeur) et les
 * interactions d'un bouton sélectionné (du bouton à chaque cible). Pointillée, indigo.
 */
export interface GhostLink {
  ...
  to: ConnectorEnd;
  label?: string;
  /** Tap sur la flèche (interaction existante) : ouvre sa bulle. */
  onTap?: (anchor: { left: number; top: number; right: number; bottom: number }) => void;
}

interface Props {
  objects: BoardObject[];
  /** Flèches éphémères (liaison en cours, interactions du bouton sélectionné). */
  ghosts?: GhostLink[];
...
export function BoardConnectorLayer({ objects, ghosts = [], scale, width, height, active, play, selectedIds, onSelect, onChange, onContextMenu, clientToUnit }: Props) {
  ...
  if (connectors.length === 0 && ghosts.length === 0) return null;
  ...
  return (
    <svg className={`wbo__connectors ${active && !play ? 'is-active' : ''}`} width={width} height={height} viewBox={`0 0 ${width / scale} ${height / scale}`}>
      {ghosts.map((gl) => {
        const g = resolveConnector({ id: gl.key, type: 'connector', x: 0, y: 0, w: 0, h: 0, from: gl.from, to: gl.to, route: 'curve', heads: { start: false, end: true }, stroke: '#6366F1', strokeWidth: 3 }, objects);
        return (
          <g key={gl.key} className={`wbo__ghostlink ${gl.onTap ? 'is-tappable' : ''}`}>
            <path d={g.path} className="wbo__ghostlink-line" fill="none" vectorEffect="non-scaling-stroke" />
            {g.heads.map((h, i) => <path key={i} d={h} className="wbo__ghostlink-head" />)}
            {gl.label && (
              <g transform={`translate(${g.mid.x} ${g.mid.y})`}>
                <rect x={-(gl.label.length * 4.6 + 8)} y={-11} width={gl.label.length * 9.2 + 16} height={22} rx={11} className="wbo__ghostlink-tag" />
                <text y={1} textAnchor="middle" dominantBaseline="middle" fontSize={14} fontWeight={700} fontFamily="Inter, system-ui, sans-serif" fill="#FFFFFF">{gl.label}</text>
              </g>
            )}
            {gl.onTap && (
              <path
                ...
                onPointerDown={(e) => {
                  ...
                  gl.onTap?.({ left: cx - 16, top: cy - 16, right: cx + 16, bottom: cy + 16 });
                }}
              />
            )}
          </g>
